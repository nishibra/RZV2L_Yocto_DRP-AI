...
uint8_t Image::get_bmp(unsigned char* c_buf)
{
    ...
    /* NOTE: Number of byte in single row of Windows Bitmap image must be aligned to 4 bytes. */
    int32_t line_width = width * channel + width % 4;
        memcpy(img_buffer,c_buf,height * width * channel);
    return 0;
}
unsigned char* Image::set_bmp()
{
    ...
    memcpy(c_buffer,img_buffer,height * width * channel);
    free(img_buffer);
    return c_buffer;
}
